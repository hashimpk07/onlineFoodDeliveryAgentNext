...
import { FONT_FAMILY_VALUES } from "@/lib/preferences/fonts";
import {
...
function readDomState(): Partial<PreferencesState> {
  ...
  const rawFontSize = getComputedStyle(root).fontSize;
  const parsedFontSize = parseInt(rawFontSize, 10);
  ...
  return {
    ...
    fontFamily: getSafeValue(
      root.getAttribute("data-font-family"),
      FONT_FAMILY_VALUES,
    ),
  };
}
