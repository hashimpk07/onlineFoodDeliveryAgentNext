...
import { updateVehicleApi } from "../_api/vehicle-update-api";
import { VehicleFormValues } from "../_types/vehicle-type";

export function useUpdateVehicle(id: string | number) {
  ...
  return useMutation({
    mutationFn: (data: VehicleFormValues) => {
      ...
      return updateVehicleApi(id, formData);
    },
    onSuccess: (res) => {
      if (res.status === "success") {
        toast.success(res.message ?? "Vehicle updated successfully");
        queryClient.invalidateQueries({ queryKey: ["vehicle"] });
        queryClient.invalidateQueries({ queryKey: ["vehicle", String(id)] });
      } else {
