...
import usePerformanceFilters from "@/app/[locale]/(main)/dashboard/3pl/reports/performance/_hooks/use-captain-filters";
...
export function FilterPanel({ table }: FilterPanelProps) {
    ...
    <div className="rounded-xl border bg-card p-4 shadow-sm sm:p-6 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <DataTableFilterBox
          ...
          filterValue={captainId.length > 0 ? captainId : null}
        ...
        <DataTableSearch
          label="Search"
          searchKey="Search by captain code or name or email or iqama"
          searchQuery={q}
          setSearchQuery={async (v, opts) => {
            const result = await setQ(v, opts);
            await setPage(1);
            return result;
          }}
        />
        ...
        <DataTableFilterBox
          ...
          filterValue={regionId.length > 0 ? regionId : null}
          setFilterValue={(value) => {
            setRegionId(value ?? []);
            setPage(1);
          }}
        ...
        <DataTableFilterBox
          ...
          filterValue={areaId.length > 0 ? areaId : null}
          setFilterValue={(value) => {
            setAreaId(value ?? []);
            setPage(1);
          }}
      ...
      <div className="flex justify-end gap-2">
        {companyId && (
          <Button
            ...
            onClick={() =>
              openExportModal({
                reportType: "3pl_captain_performance_report",
